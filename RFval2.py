...
pickle_file = "C:\\data_for_learning\\RegressionGW.pickle" ### Write path for the full_shape_val_data.pkl file ###
with open(pickle_file, 'rb') as f:
    reg = pickle.load(f)
...
for i in range(100):
        if objectlist[i]['semantic class'] == 'Bottle':
            ...
            Aff_g=np.asarray(np.asarray(pcd.colors)[:, :1])
            Aff_W=np.asarray(np.asarray(pcd.colors)[:, 1:2])
            ...
            np_colors=(np.concatenate((aff[:,1:2],np_colors),axis=1))
            pcd.colors=o3d.utility.Vector3dVector(np_colors)
